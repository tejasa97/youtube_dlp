...
func Parse(manifestURL string, body []byte) (Manifest, error) {
	...
	for _, stream := range source.Streams {
		if stream.Type == "" || stream.URL == "" || len(stream.Qualities) == 0 {
		...
		for _, chunk := range stream.Chunks {
			...
			current += chunk.Duration * int64(chunk.Repeat+1)
		}
		if len(chunks) == 0 {
			return Manifest{}, fmt.Errorf("%w: stream without chunks", ErrInvalidManifest)
		}
		...
		result.Streams = append(result.Streams, Stream{Type: strings.ToLower(stream.Type), URL: stream.URL, Qualities: qualities, Chunks: chunks})
	}
	return result, nil
}
...
func Address(manifestURL string, manifest Manifest, stream Stream, limit int) ([]Segment, error) {
	...
	for chunkIndex, chunk := range stream.Chunks {
		...
		if repeat < 0 { // Smooth manifests use r=-1 through the next explicit t or presentation duration.
			...
			repeat = int((end-chunk.Time)/chunk.Duration) - 1
		}
		if repeat < 0 || repeat+1 > limit-len(result) {
			return nil, ErrTimelineBound
		}
		for iteration := 0; iteration <= repeat; iteration++ {
			time := chunk.Time + int64(iteration)*chunk.Duration
