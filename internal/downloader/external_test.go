...
func TestExternalAdapterUsesArgvAndCategorizesFailures(t *testing.T) {
	...
	result, err := NewExternalAdapter(process).Download(context.Background(), ExternalRequest{Executable: "sh", Arguments: []string{"-c", "ignored"}, URL: "https://example.test/media?token=secret", OutputRoot: root, Destination: destination})
	...
	_, err = NewExternalAdapter(process).Download(context.Background(), ExternalRequest{Executable: "sh", URL: "https://example.test", OutputRoot: root, Destination: filepath.Join(root, "bad")})
	if !errors.Is(err, ErrExternalFailed) {
		t.Fatalf("error=%v", err)
	}
	result, err = NewExternalAdapter(process).Download(context.Background(), ExternalRequest{Executable: "sh", URL: "https://example.test", OutputRoot: root, Destination: filepath.Join(root, "bad-again")})
...
func TestExternalAdapterRejectsUnsafeInputs(t *testing.T) {
	root := t.TempDir()
	_, err := NewExternalAdapter(&recordingProcess{}).Download(context.Background(), ExternalRequest{Executable: "sh", URL: "x\ny", OutputRoot: root, Destination: filepath.Join(root, "x")})
