...
import (
	...
	compatconfig "github.com/tejasa97/ytdlp-go/internal/compat/config"
...
func runContextIOWithDependencies(ctx context.Context, args []string, stdin io.Reader, stdout, stderr io.Writer, deps runDependencies) int {
	...
	splitChapters := flags.Bool("split-chapters", false, "split media into bounded files using internal chapters")
	flags.BoolFunc("no-split-chapters", "disable chapter splitting", func(input string) error {
		enabled, err := strconv.ParseBool(input)
		if err != nil {
			return err
		}
