...
func TestBuildSponsorBlockOptionsMapping(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "exclusion empties set", markSpecs: []string{"sponsor,-sponsor"}, wantErr: "sponsorblock-mark requires at least one category"},
...
func TestRunSponsorBlockConfigCommandLinePrecedence(t *testing.T) {
	...
	t.Run("cli mark overrides config no-sponsorblock", func(t *testing.T) {
		configPath := filepath.Join(t.TempDir(), "yt-dlp.conf")
		if err := os.WriteFile(configPath, []byte("--no-sponsorblock\n--sponsorblock-api https://sponsor.example.test\n"), 0o600); err != nil {
			t.Fatal(err)
		}
		// Config disables first; CLI mark re-enables. Non-YouTube URL must fail
		// closed with unsupported once marking is enabled again.
		var stdout, stderr bytes.Buffer
		code := Run([]string{
			"--config-location", configPath,
		...
		if code == 0 {
			t.Fatalf("expected unsupported enablement after CLI mark, stderr=%q", stderr.String())
		}
		rendered := stderr.String()
		if !strings.Contains(rendered, "unsupported") && !strings.Contains(rendered, "sponsorblock") {
			t.Fatalf("stderr = %q, want sponsorblock unsupported", rendered)
		}
	})
