...
func TestYouTubeExtractsManualAutomaticTranslatedAndProtectedCaptions(t *testing.T) {
	...
	result, err := NewYouTube().Extract(context.Background(), Request{
		URL: youtubeFixtureURL, Transport: transport, Options: Options{POT: director},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	translated, err := NewYouTube().Extract(context.Background(), Request{
		URL: youtubeFixtureURL, Transport: transport, Options: Options{POT: director, TranslatedCaptions: true},
	})
	if err != nil {
		t.Fatal(err)
	}
