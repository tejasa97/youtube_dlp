...
// RequestJSONWithExecutor performs the same bounded, redacted JSON exchange
// through a caller-supplied operation-scoped executor. Provider authentication
// flows use this only after selecting the required transport isolation mode.
func RequestJSONWithExecutor(ctx context.Context, execute func(context.Context, *http.Request) (*http.Response, error), method, rawURL string, body []byte, headers http.Header, target any) error {
	return requestJSON(ctx, execute, method, rawURL, body, headers, target)
}
