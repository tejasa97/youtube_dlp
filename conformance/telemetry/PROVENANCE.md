...
This is a foundation-package fixture, not a product `telemetrycheck` input:
it intentionally exercises `download` and `live` capability dimensions in
addition to `extract`. Product snapshots currently admit only `extract`; a
deployment coverage report must be generated by the product collector rather
than substituting this synthetic fixture.
