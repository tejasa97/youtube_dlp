...
| Wimbledon | `yt_dlp/extractor/wimbledon.py` / `WimbledonIE` | Metadata API `wim_v1_<id>_en`; account `3506358525001`; title + duration overlay | Bounded JSON; duration colon/seconds/ms parsing; description omitted (Entry contract) |
...
| Sky News AU | `yt_dlp/extractor/skynewsau.py` / `SkyNewsAUIE` | `embedcode="account-video"`; News API `content.api.news`; caption + `date.created` | API key never echoed in errors; strict embedcode/account/video validation |
