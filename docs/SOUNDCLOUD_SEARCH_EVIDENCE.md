...
Deliberate hardening beyond the reference: a 500-byte query maximum, 4 KiB target/continuation maximum, a 200-entry response page maximum, exact HTTPS `api-v2.soundcloud.com/search/tracks` continuations, and a 200-result `all` cap. Every cursor may contain exactly one each of `q`, `limit`, `linked_partitioning`, and `offset`, plus at most one nonempty `cursor`; stale `client_id` is removed before request and every other key or duplicate fails closed. Cross-origin, downgrade, userinfo, ports, fragments, encoded separators, malformed query encodings, and replayed continuation URLs fail closed without echoing cursor/query data.

An API page is requested with the desired result count as its page size, hence a populated compliant page can satisfy the request in one call. The implementation permits four API pages (one expected page plus three empty/unsupported-result tolerance pages) and then returns `ErrPlaylistLimit`; it never inherits the generic 10,000-page continuation ceiling. Search API JSON calls use `RequestJSONWithoutCookies`, which requires the transport's real `DoWithoutCookies` capability. First-party client-ID bootstrap remains on the existing bounded SoundCloud bootstrap path. A transport without cookie isolation fails closed with `ErrTransportIsolation`.

Only explicit `kind:"track"` entries with a positive stable ID, non-empty title, and a canonical public `https://soundcloud.com/<artist>/<track>` track URL are emitted. Users (including track-shaped user URLs), playlists, malformed candidates, private-token URLs, API URLs, and off-origin URLs are skipped.
...
3. Before every request, continuation validation returns a local canonical URL with exact query-key/cardinality policy; rejected URLs do not alter iterator cursor/page state. `client_id` is stripped then re-added locally, with no cursor/query value included in diagnostics.
...
5. A response is decoded and fully validated into local page/entry slices before `ContinuationEntries` commits the next iterator page. The private search sequence has a four-fetch cap independent of emitted entries. Fetch failure marks only that iterator done; other iterators can restart independently.
