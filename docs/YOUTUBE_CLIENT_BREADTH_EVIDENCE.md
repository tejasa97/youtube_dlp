...
maintenance `69ea20006` (VISIONOS-first logged-out recovery), `dae52d83`
(#17461, `android_vr` removed from the default clients because since
2026.08.17 it returns HTTP 403 for all formats at version 1.65.10), and
`5d5b634d` (#17462, `web_embedded` added to the authenticated defaults and the
VR-class fallback; premium reordered to `web_creator`-first).
...
android/ios/mweb recovery clients already pinned in this port). `android_vr`
is intentionally absent from this default rotation (`dae52d83` / #17461); it is
retained as a fail-closed, non-default profile (`youtubeAndroidVRClient`).
...
`visionos` (or an explicitly selected `android_vr`) append a single
`web_embedded` then `tv_downgraded` retry when JavaScript support is present
(`5d5b634d` / #17462). That fallback is not part of authenticated recovery.
...
Retained non-default profile: `android_vr` — ANDROID_VR / 28 / 1.65.10 (GVS
fail-closed; excluded from the default rotation by #17461).
...
2. Then, matching current upstream defaults after `5d5b634d` (#17462):
   - Premium (`_DEFAULT_PREMIUM_CLIENTS`): `web_creator`, `tv_downgraded`, `web`
   - Authenticated non-Premium (`_DEFAULT_AUTHED_CLIENTS`): `web_embedded`,
     `tv_downgraded`, `web`
...
3. Exact pinned identities:
   - `web_embedded` — WEB_EMBEDDED_PLAYER / 56 / 2.20260708.00.00 (cookie-capable;
     no required GVS policy; can work around age / embeddable restrictions)
...
The retained non-default `android_vr` profile requires a GVS PO token for
adaptive formats unless a player token has already waived that requirement
(`NotRequiredWithPlayerToken`). Missing or rejected required tokens drop those
formats instead of advertising URLs that immediately return HTTP 403. It is no
longer part of the default rotation (#17461).
