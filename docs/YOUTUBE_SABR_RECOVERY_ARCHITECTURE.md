...
| `youtubepot.Episode` | Operation-scoped forced-refresh coordination for compatible A/V | Same Director cache key ⇒ shared flight/refresh; incompatible keys never share | One pending bypass per `SignalRejection`; total forced ≤ 2/op | Product requirement; not a second cache |
...
| `youtubeump.Downloader` recovery | Transactional response consume; HTTP retries reuse immutable body; SABR_ERROR retry vs reload callback | Committed cookie/redirect/contexts/token/inventory untouched on failed response | HTTP `Attempts` (existing); `MaxSabrErrorRecoveries` = 3; `MaxReloadAttempts` = 2 | LuanRT `SabrStream.ts` retry + `SabrStreamingAdapter.ts` reload |
| Product SABR refresh coordinator | Re-extract via existing YouTube extractor; accept only exact video/client/visitor/itag/track/format/duration | Exact signed query bytes preserved; trusted-host / no-redirect / credential isolation | `MaxSabrRefreshAttempts` = 2; compatible A/V share one extraction | Product live-refresh pattern scoped to SABR markers |
...
| Canonical `RELOAD_PLAYER_RESPONSE` with nonempty reload token | Reload via typed callback; replace inventory atomically; separate reload budget | Matches davidzeng0 “new `/player` request” and GoogleVideo reload event. |
...
| `youtubepot.ErrTokenRejected` | Arms ≤1 forced cache bypass on the operation `Episode` | Callers must have an attributable rejection; SABR_ERROR alone does not invent POT rejection. |
