...
| Product SABR refresh coordinator | Context-aware keyed extraction flight with waiter refcount/abandon; accept only exact identity match | Video, client name/ID/version, visitor, itag, track, duration, DRC, audio track, ustreamer hash; exact signed query bytes preserved | `MaxSabrRefreshAttempts` = 2; compatible A/V share one extraction; canceled waiters return promptly; last waiter cancels shared work | Product live-refresh pattern scoped to SABR markers |
