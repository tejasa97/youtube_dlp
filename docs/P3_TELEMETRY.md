...
is mutually exclusive with `--print-json`, `--dump-json`, and
`--dump-single-json`, records failed operations before the CLI returns, and
never writes a URL or error message to the snapshot.
