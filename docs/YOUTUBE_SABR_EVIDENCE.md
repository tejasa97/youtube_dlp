...
- `RELOAD_PLAYER_RESPONSE` (46): canonical LuanRT `ReloadPlaybackContext` → nested `ReloadPlaybackParams.token` (field 1 → field 1, nonempty ≤4096 bytes, valid UTF-8). Duplicate/empty/oversized/invalid-UTF-8 tokens fail closed. Emits a redacted `ReloadPlayerSignal` (token never appears in `Error`/formatting). Recovery calls an explicit typed reload callback that places the token at `playbackContext.reloadPlaybackContext.reloadPlaybackParams.token` via `extractor.ReloadYouTubePlayer` (LuanRT/googlevideo sabr-shaka-example @ `d2fa40d7`), which re-validates UTF-8 before JSON marshalling so `encoding/json` cannot silently mutate opaque token bytes. Accepted material requires identical video ID and exact client name/ID/version, visitor, itag/track/duration, DRC/audio-track, and ustreamer SHA-256 equality (committed checkpoint bytes are never rebound to a silently changed ustreamer). Atomically replaces signed URL (+ optional PO token), clears playback cookie/contexts, resets `rn` to 0, and is capped at `MaxReloadAttempts` (2). davidzeng0 alternate unnamed nested wrappers are unsupported.
...
- Signed extraction refresh: operation-scoped product coordinator uses a context-aware keyed extraction flight with waiter reference counting (mutex covers bookkeeping only; last waiter cancels shared work). Compatible A/V identities sharing video/client name/ID/version/visitor share one extraction attempt (`MaxSabrRefreshAttempts` = 2); per-itag materials remain separate. Accepted material must match video, exact client name/ID/version, visitor, itag, track, duration, DRC/audio-track, and ustreamer SHA-256 (identity key includes decoded ustreamer hash; malformed expected base64 fails closed); missing required identity values fail closed; exact signed query bytes are preserved; trusted-host / no-redirect / credential isolation is enforced. Resume refresh is fail-closed when a `Refresh` callback is wired (product path with extract-capable client+transport): callback or identity failure must not fall back to stale supplied inventory. The only safe continue-with-caller-material case is when `Refresh` is not wired. `RELOAD_PLAYER_RESPONSE` uses the focused reload player API and is hard-fail. Mid-session PO tokens resolve through `youtubepot.Director` with expiry skew and per-key reference-counted single-flight (bypass flights never join normal flights; canceled creators/waiters return promptly without poisoning survivors). `Episode.SignalRejection` remains an extension hook for attributable `ErrTokenRejected` from embedding callers — not product-integrated SABR recovery.
