...
| Manifest subtitle fallback merge and `_credential_isolated` metadata | `TestVimeoSubtitleManifestFallbackMergesHLSAndDASH`, `internal/protocol/hls.TestParseMasterSubtitles`, `internal/protocol/hls.TestAssembleWebVTTConcatenatesSegments`, `internal/protocol/hls.TestAssembleWebVTTRejectsEncryptedSegments`, `internal/protocol/dash.TestParseTextRepresentations`, `internal/downloader.TestWriteFinalizesPayloadAtomically`, `engine.TestSubtitleDownloaderAssemblesHLSSubtitlePlaylists`, `engine.TestSubtitleHLSDownloadRejectsEncryptedPlaylists` |
...
| Per-track product isolation and ambient redirect regression | `engine.TestSubtitleHLSIsolatedTrackSendsNoCredentials`, `engine.TestSubtitleIsolatedDirectDownloadSendsNoCredentials`, `engine.TestCredentialIsolatedSubtitleTransportStripsAmbientCredentials`, `engine.TestSubtitleHLSPreservesAmbientCredentialsAndRedirects`, `engine.TestSubtitleDirectDownloadPreservesAmbientCredentialsAndRedirects`, `engine.TestSubtitleDASHDownloadPreservesAmbientCredentials`, `internal/network.TestDoWithoutCredentialsNoRedirectDropsAllCredentialSources` |
