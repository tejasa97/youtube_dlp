...
func TestLoadInfoJSONDownloadsBoundedMetadataWithoutAmbientCredentials(t *testing.T) {
	...
	_, err = newBroadTestClient().Run(context.Background(), Request{LoadInfoJSON: input, CookieFile: input})
...
func TestLoadInfoJSONRejectsUnsafeShapesBoundsAndCancellation(t *testing.T) {
	...
	t.Run("wrong root", func(t *testing.T) {
		...
		_, err := newBroadTestClient().Run(context.Background(), Request{LoadInfoJSON: path})
	...
	t.Run("cancelled", func(t *testing.T) {
		...
		cancel()
		_, err := newBroadTestClient().Run(ctx, Request{LoadInfoJSON: path})
...
func TestAutonumberCountsOnlyAcceptedPlaylistEntries(t *testing.T) {
	...
	operation := &operation{
		...
		registry: legacyRuntime(autonumberLifecycleExtractor{}),
	}
