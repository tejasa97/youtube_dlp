...
func TestRouteRequestInputURLAndSearchClassification(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			got, err := routeRequestInput(context.Background(), wrapLegacyRegistry(registry), Request{
...
func TestRouteRequestInputCancellationAndConcurrency(t *testing.T) {
	...
	cancel()
	if _, err := routeRequestInput(cancelled, wrapLegacyRegistry(registry), Request{URL: "plain term", DefaultSearch: "auto"}); !errors.Is(err, context.Canceled) {
	...
	for index := 0; index < workers; index++ {
		...
		go func(index int) {
			...
			got, err := routeRequestInput(context.Background(), wrapLegacyRegistry(registry), Request{URL: query, DefaultSearch: "ytsearch"})
...
func TestClientForceGenericUsesRegisteredGenericExtractor(t *testing.T) {
	...
	defer server.Close()
	result, err := newBroadTestClient().Run(context.Background(), Request{URL: server.URL + "/media.mp4", ForceGenericExtractor: true, SkipDownload: true})
	if err != nil {
		t.Fatal(err)
	}
...
func TestDefaultSearchRequestOverrideIsTypedAndSafe(t *testing.T) {
	registry := productRegistry()
	routed, err := routeRequestInput(context.Background(), wrapLegacyRegistry(registry), Request{URL: "cats & dogs? token=secret", DefaultSearch: "ytsearch"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestOperationSearchQueryOverrideIsRootOnlyForURLResultChildren(t *testing.T) {
	...
	operation := &operation{
		...
		registry:           wrapLegacyRegistry(registry),
...
func TestRouteRequestInputPluginIDPreservesOriginalAndBypassesBuiltIns(t *testing.T) {
	...
	routed, err := routeRequestInput(context.Background(), wrapLegacyRegistry(extractor.NewRegistry(spy)), Request{
