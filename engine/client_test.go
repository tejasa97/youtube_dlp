...
import (
	...
	"github.com/tejasa97/ytdlp-go/internal/compat/matchfilter"
	"github.com/tejasa97/ytdlp-go/internal/cookies/chromium"
	"github.com/tejasa97/ytdlp-go/internal/cookies/chromiumlinux"
	"github.com/tejasa97/ytdlp-go/internal/cookies/chromiumwindows"
	"github.com/tejasa97/ytdlp-go/internal/cookies/firefox"
	"github.com/tejasa97/ytdlp-go/internal/cookies/netscape"
	"github.com/tejasa97/ytdlp-go/internal/cookies/safari"
	...
	"github.com/tejasa97/ytdlp-go/internal/media/pipeline"
...
func TestProductAeonCoPropagatesRefererThroughURLRecursion(t *testing.T) {
	...
	op := &operation{
		...
		registry:      legacyRuntime(extractor.NewAeonCo(), recorder),
...
func TestExtractorFailuresAreCategorized(t *testing.T) {
	...
	if class, ok := broadTestClassifyProviderError(extractor.ErrTwitchSubscriberOnly); !ok || class != providerapi.ErrorAuthentication {
		t.Fatalf("broad Twitch classification = %q, %v", class, ok)
	}
}
...
func TestProductRegistryIncludesIntegratedExtractors(t *testing.T) {
	tests := []struct {
		...
		{"https://player.bilibili.com/player.html?aid=92494333&cid=157926707&page=1", "bilibili_player"},
		{"https://t.bilibili.com/998134289197432852", "bilibili_dynamic"},
		{"https://www.bilibili.com/opus/12345", "bilibili_dynamic"},
		{"https://www.bilibili.tv/en/play/12345", "biliintl_series"},
		{"https://www.bilibili.tv/en/play/12345/67890", "biliintl"},
		{"https://www.biliintl.com/play/12345", "biliintl_series"},
		{"https://space.bilibili.com/2142762/lists/3662502", "bilibili_collection"},
		{"https://space.bilibili.com/1958703906/lists/547718?type=series", "bilibili_series"},
		{"https://www.bilibili.com/bangumi/media/md24097891", "bilibili_bangumi_media"},
		{"https://www.bilibili.com/bangumi/play/ss26801", "bilibili_bangumi_season"},
		{"https://www.bilibili.com/bangumi/play/ep21495", "bilibili_bangumi"},
		{"https://www.bilibili.com/audio/am10624", "bilibili_audio_album"},
		{"https://www.bilibili.com/audio/au12345", "bilibili_audio"},
		{"https://www.bilibili.com/v/kichiku/mad", "bilibili_category"},
...
func TestProductPRXMultipartReentryTerminates(t *testing.T) {
	...
	root := ""
	op := &operation{client: newBroadTestClient(), request: request, transport: transport, registry: legacyRuntime(extractor.NewPRXStory()), compatibility: plan, rootExtractor: &root}
...
func TestProductPRXAccountReentryOrder(t *testing.T) {
	...
	op := &operation{client: newBroadTestClient(), request: req, transport: transport, registry: legacyRuntime(extractor.NewPRXAccount(), extractor.NewPRXSeries(), extractor.NewPRXStory()), compatibility: plan}
...
func TestClientAppliesMatchFilterBeforeMetadata(t *testing.T) {
	...
	defer server.Close()
	var events []Event
	result, err := newBroadTestClient(WithEventHandler(func(_ context.Context, event Event) error {
...
func TestClientInteractiveFormatRepromptsThenSelects(t *testing.T) {
	...
	operation := &operation{client: newBroadTestClient(), request: request, registry: legacyRuntime(interactiveFormatExtractor{}), compatibility: plan}
...
func TestClientDefersExpensiveMetadataUntilAfterMatchFilter(t *testing.T) {
	for _, test := range []struct {
		...
		operation := &operation{
			client: newBroadTestClient(), request: request,
			registry:      legacyRuntime(deferredMetadataExtractor{calls: &calls}),
...
func TestClientCategorizesMatchFilterEvaluationFailure(t *testing.T) {
	...
	operation := &operation{
		client: newBroadTestClient(), request: request, registry: legacyRuntime(numericMetadataExtractor{}),
		compatibility: plan,
	}
...
func TestClientInteractiveMatchFilterAcceptRejectAndOrdering(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			operation := &operation{
				client: newBroadTestClient(), request: request, registry: legacyRuntime(interactiveFormatExtractor{}),
				compatibility: plan,
			}
			result, err := operation.process(context.Background(), request.URL, "", nil, make(map[string]bool), 0)
			if err != nil {
...
func TestOperationPostLivePreferenceKeepsExplicitDirectFormatAuthoritative(t *testing.T) {
	...
	explicitOperation := &operation{
		client: newBroadTestClient(), request: request, transport: transport, compatibility: compatibility,
	}
...
func TestClientCancellationReachesTransport(t *testing.T) {
	...
	defer cancel()
	_, err := newBroadTestClient().Run(ctx, Request{URL: server.URL + "/slow?delay=1s", SkipDownload: true})
...
func TestClientDownloadArchiveRecordsAndSkips(t *testing.T) {
	...
	first, err := newBroadTestClient().Run(context.Background(), request)
	...
	var events []Event
	second, err := newBroadTestClient(WithEventHandler(func(_ context.Context, event Event) error {
		events = append(events, event)
		return nil
	})).Run(context.Background(), request)
...
func TestClientInitializesConfiguredCacheSafely(t *testing.T) {
	...
	result, err := newBroadTestClient().Run(context.Background(), Request{URL: server.URL + "/page", CacheDir: cacheRoot, SkipDownload: true})
	...
	if err := os.Symlink(cacheRoot, unsafe); err == nil {
		_, err = newBroadTestClient().Run(context.Background(), Request{URL: server.URL + "/page", CacheDir: unsafe, SkipDownload: true})
		if !IsCategory(err, ErrorInvalidInput) {
...
func TestClientHLSAndDASHDispatch(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			result, err := newBroadTestClient().Run(context.Background(), Request{URL: server.URL + test.path, OutputDir: t.TempDir()})
			if err != nil {
				t.Fatal(err)
			}
			contents, err := os.ReadFile(result.Filename)
...
func TestClientISMDispatch(t *testing.T) {
	...
	defer server.Close()
	root := t.TempDir()
	result, err := newBroadTestClient().Run(context.Background(), Request{URL: server.URL + "/Manifest", OutputDir: root})
	if err != nil {
		t.Fatal(err)
	}
	contents, err := os.ReadFile(result.Filename)
...
<AdaptationSet contentType="audio" mimeType="audio/mp4"><Representation id="audio" bandwidth="128"><BaseURL>audio.m4a</BaseURL></Representation></AdaptationSet>
	...
	defer cancel()
	result, err := newBroadTestClient().Run(ctx, Request{URL: server.URL + "/manifest.mpd", OutputDir: root, Overwrite: true})
	if err != nil {
		t.Fatal(err)
	}
	tools, err := ffmpeg.Discover(ffmpeg.Config{})
...
func TestOperationResolvesNestedPlaylistInOrder(t *testing.T) {
	...
	operation := &operation{
		client: newBroadTestClient(), request: Request{SkipDownload: true}, transport: transport,
...
func TestOperationDownloadsPlaylistAndAggregatesBytes(t *testing.T) {
	...
	root := t.TempDir()
	operation := &operation{
		client: newBroadTestClient(), request: Request{OutputDir: root}, transport: transport,
		registry: legacyRuntime(playlistFixtureExtractor{}, extractor.NewGeneric()),
	}
	result, err := operation.process(context.Background(), server.URL+"/list", "", nil, make(map[string]bool), 0)
	if err != nil {
...
func TestOperationMergesTransparentEntryMetadata(t *testing.T) {
	...
	operation := &operation{
		client: newBroadTestClient(), request: Request{SkipDownload: true}, transport: transport,
		registry: legacyRuntime(extractor.NewGeneric()),
	}
...
func TestOperationTransparentZeroNumericsSurviveURLResultHandoff(t *testing.T) {
	...
	operation := &operation{
		client: newBroadTestClient(), request: Request{SkipDownload: true}, transport: transport,
		registry: legacyRuntime(zeroNumericsHandoffParent{}, extractor.NewGeneric()),
	}
...
func TestOperationMergesTransparentParentInfoFromURLResult(t *testing.T) {
	...
	operation := &operation{
		client: newBroadTestClient(), request: Request{SkipDownload: true}, transport: transport,
		registry: legacyRuntime(transparentParentFixtureExtractor{}, extractor.NewGeneric()),
	}
...
func amaraProductOperation(t *testing.T, transport *network.Client, extra ...extractor.Extractor) *operation {
	...
	return &operation{
		client: newBroadTestClient(), request: Request{SkipDownload: true}, transport: transport,
		registry: legacyRuntime(extractors...),
	}
}
...
func TestOperationConcurrentMaxDownloadsAdmission(t *testing.T) {
	...
	operation := &operation{
		...
		registry: legacyRuntime(mediaExtractor),
	}
	urls := []string{
...
func TestOperationURLResultBypassesPlaylistControls(t *testing.T) {
	...
	operation := &operation{
		...
		transport: transport,
		registry:  legacyRuntime(URLResultFixtureExtractor{}, extractor.NewGeneric()),
	}
...
func TestOperationRejectsURLResultCycle(t *testing.T) {
	operation := &operation{
		client: newBroadTestClient(), request: Request{SkipDownload: true},
		registry: legacyRuntime(cyclicURLResultExtractor{}),
	}
...
func TestOperationPropagatesRefererThroughPlaylistRecursion(t *testing.T) {
	...
	operation := &operation{
		client: newBroadTestClient(), request: Request{SkipDownload: true}, transport: transport,
		registry: legacyRuntime(refererPlaylistExtractor{}, child),
	}
...
func TestProductDiscoveryHLSAndDASHDownloadDispatch(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			operation := &operation{client: newBroadTestClient(), request: request, transport: transport, registry: productRuntime(), rootExtractor: new(string)}
	...
	for _, site := range []struct {
		...
		for _, media := range []struct {
			...
			t.Run(site.name+"/"+media.name, func(t *testing.T) {
				...
				operation := &operation{
					client: newBroadTestClient(), request: Request{OutputDir: t.TempDir()}, transport: transport,
					registry: productRuntime(), rootExtractor: new(string),
				}
...
func TestProductCategorizesDiscoveryFailures(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			operation := &operation{client: newBroadTestClient(), request: Request{SkipDownload: true}, transport: transport, registry: productRuntime(), rootExtractor: new(string)}
