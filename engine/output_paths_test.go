...
func TestClientRejectsUnsafeOutputPathsBeforeExtractionAndClonesMap(t *testing.T) {
	...
	_, err := newBroadTestClient().Run(context.Background(), Request{URL: server.URL + "/page", OutputPaths: paths})
	...
	cancel()
	_, err = newBroadTestClient().Run(ctx, Request{URL: server.URL + "/page", OutputPaths: OutputPaths{OutputPathHome: t.TempDir()}})
