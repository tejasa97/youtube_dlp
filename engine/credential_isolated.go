...
func newCredentialIsolatedTransportWithReferer(ambient *network.Client, referer string) (*credentialIsolatedTransport, error) {
	if referer != "" && !validCredentialIsolatedMediaReferer(referer) {
		return nil, fmt.Errorf("%w: invalid scoped media referer", ErrTransportIsolation)
	}
	if ambient == nil {
		return nil, nil
	}
...
func (operation *operation) mediaTransport(credentialIsolated bool, referer, hostPolicy, protocol string) (any, error) {
	if hostPolicy != "" {
		if referer != "" {
			return nil, fmt.Errorf("%w: scoped referer cannot be combined with host policy", ErrTransportIsolation)
		}
		if !credentialIsolated {
			return nil, fmt.Errorf("%w: provider host policy requires credential isolation", ErrTransportIsolation)
		}
		role := "media"
		if protocol == "m3u8_native" {
			role = "playback"
		}
		return newProviderPolicyTransport(operation, hostPolicy, role), nil
	}
	if !credentialIsolated {
		if referer != "" {
			return nil, fmt.Errorf("%w: scoped referer requires credential isolation", ErrTransportIsolation)
		}
