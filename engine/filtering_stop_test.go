...
func TestClientBreakOnRejectStops(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			result, err := newBroadTestClient().Run(context.Background(), test.request)
			if err != nil {
				t.Fatal(err)
			}
...
func TestClientArchivePrecedesFilters(t *testing.T) {
	...
	if _, err := newBroadTestClient().Run(context.Background(), base); err != nil {
		t.Fatal(err)
	}
	// The second run's filter would reject, but the archive match takes
	// precedence: the result reports Archived, never Skipped.
	second, err := newBroadTestClient().Run(context.Background(), Request{
	...
	third, err := newBroadTestClient().Run(context.Background(), Request{
		URL: server.URL + "/page", OutputDir: root, DownloadArchive: archivePath, SkipDownload: true,
		MatchFilters:    []string{"title=other"},
...
func TestClientMaxDownloadsCountsPassingEntries(t *testing.T) {
	...
	result, err := newBroadTestClient().Run(context.Background(), Request{URL: server.URL + "/page", OutputDir: root, SkipDownload: true, MaxDownloads: 1})
	if err != nil {
		t.Fatal(err)
	}
	...
	result, err = newBroadTestClient().Run(context.Background(), Request{URL: server.URL + "/page", OutputDir: root, SkipDownload: true, MaxDownloads: 2})
	if err != nil {
		t.Fatal(err)
	}
	...
	result, err = newBroadTestClient().Run(context.Background(), Request{URL: server.URL + "/page", OutputDir: root, Simulate: true, MaxDownloads: 1})
	if err != nil {
		t.Fatal(err)
	}
	if result.Downloads != 1 || !result.Stopped || result.StopKind != StopMaxDownloads {
...
func TestClientMaxDownloadsDoesNotCountArchiveOrRejectedEntries(t *testing.T) {
	...
	root := t.TempDir()
	archivePath := filepath.Join(root, "archive.txt")
	if _, err := newBroadTestClient().Run(context.Background(), Request{
		URL: server.URL + "/page", OutputDir: root, DownloadArchive: archivePath,
	}); err != nil {
		t.Fatal(err)
	}
	archived, err := newBroadTestClient().Run(context.Background(), Request{
	...
	rejected, err := newBroadTestClient().Run(context.Background(), Request{
...
func TestOperationMaxDownloadsStopsPlaylist(t *testing.T) {
	...
	transport, _ := network.New(network.Config{})
	operation := &operation{
		client: newBroadTestClient(), request: Request{OutputDir: t.TempDir(), MaxDownloads: 1}, transport: transport,
