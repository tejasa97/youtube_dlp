...
func runFailingSelectionWithHandler(
	...
	var mu sync.Mutex
	var events []Event
	client := newBroadTestClient(WithEventHandler(func(handlerCtx context.Context, event Event) error {
		mu.Lock()
	...
	operation := &operation{
		...
		transport: transport,
		registry:  legacyRuntime(fixture, &failEntryExtractor{err: fixture.failError}, extractor.NewGeneric()),
	}
...
func TestPlaylistLazyPlusReverseEmitsWarningAndDoesNotReverse(t *testing.T) {
	...
	operation := &operation{
		...
		transport: transport,
		registry:  legacyRuntime(&selectionFixtureExtractor{pageFetches: &pages}, extractor.NewGeneric()),
	}
...
func TestPlaylistContinuePreservesPrintLifecycleForDownloadError(t *testing.T) {
	...
	operation := &operation{
		...
		registry: legacyRuntime(failingMediaExtractor{mediaURL: server.URL}),
	}
