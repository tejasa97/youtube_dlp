...
type youtubeSABRRefreshCoordinator struct {
	operation *operation
	episode   providerapi.POTEpisodeResolver
	...
	extract      func(context.Context, string) (Extraction, error)
	reloadPlayer func(context.Context, mediaformat.Selection, string) (Extraction, error)
}
...
func newYouTubeSABRRefreshCoordinator(operation *operation) *youtubeSABRRefreshCoordinator {
	coordinator := &youtubeSABRRefreshCoordinator{
		operation: operation,
		episode:   operation.newPOTEpisodeResolver(),
...
func (coordinator *youtubeSABRRefreshCoordinator) refresh(ctx context.Context, selection mediaformat.Selection) (youtubeump.RefreshMaterial, error) {
	return coordinator.runFlight(ctx, selection, youtubeSABRExtractionGroup(selection), func(runCtx context.Context) (Extraction, error) {
		sourceURL := selection.YouTubeSourceURL
		if sourceURL == "" {
			return Extraction{}, youtubeump.ErrRefreshRejected
		}
...
func (coordinator *youtubeSABRRefreshCoordinator) reload(ctx context.Context, selection mediaformat.Selection, token string) (youtubeump.RefreshMaterial, error) {
	...
	return coordinator.runFlight(ctx, selection, group, func(runCtx context.Context) (Extraction, error) {
...
func (coordinator *youtubeSABRRefreshCoordinator) runFlight(
	...
	group string,
	lead func(context.Context) (Extraction, error),
...
func (coordinator *youtubeSABRRefreshCoordinator) storeExtraction(ctx context.Context, selection mediaformat.Selection, extracted Extraction) error {
...
func sabrMaterialFromObject(ctx context.Context, operation *operation, selection mediaformat.Selection, object *value.Object, episode providerapi.POTEpisodeResolver) (youtubeump.RefreshMaterial, bool, error) {
...
func (coordinator *youtubeSABRRefreshCoordinator) extractSource(ctx context.Context, sourceURL string) (Extraction, error) {
	operation := coordinator.operation
	if operation == nil || operation.client == nil || operation.transport == nil {
		return Extraction{}, youtubeump.ErrRefreshRejected
	}
	return operation.extractProviderSource(ctx, sourceURL)
}

func (coordinator *youtubeSABRRefreshCoordinator) reloadSource(ctx context.Context, selection mediaformat.Selection, token string) (Extraction, error) {
	operation := coordinator.operation
	if operation == nil || operation.client == nil || operation.transport == nil {
		return Extraction{}, youtubeump.ErrReloadRejected
	}
	...
	reload := operation.registry.Hooks().Reload
	if reload == nil {
		return Extraction{}, youtubeump.ErrReloadRejected
	}
	return reload(ctx, operation.providerRequest(selection.YouTubeSourceURL, "", ""), operation.client.compositionState(operation.request), providerapi.ReloadRequest{
		MediaID:         selection.YouTubeSABRVideoID,
		...
		DurationSeconds: selection.YouTubeSABRDurationSec,
	})
}
...
func resolveYouTubeSABRPOTokenEpisode(ctx context.Context, operation *operation, selected mediaformat.Selection, episode providerapi.POTEpisodeResolver) ([]byte, error) {
	if operation == nil || operation.client == nil || operation.client.potResolver == nil || episode == nil {
		return nil, nil
	}
	token, ok, err := episode.Resolve(ctx, providerapi.POTRequest{
