...
func selectThumbnails(info *value.Info) ([]thumbnailTrack, error) {
	...
	if hasList && len(raw) > maxThumbnails {
		return nil, fmt.Errorf("%w: thumbnail limit", ErrInvalidMetadata)
	}
	...
	for _, track := range tracks {
		...
		if writeAll {
			if _, duplicate := seen[final]; duplicate {
				return nil, 0, fmt.Errorf("%w: duplicate thumbnail destination", ErrInvalidMetadata)
			}
			seen[final] = struct{}{}
		}
	}
	...
	for index := len(tracks) - 1; index >= 0; index-- {
		...
		if writeAll {
			if _, duplicate := committedPaths[destination]; duplicate {
				return artifacts, total, fmt.Errorf("%w: duplicate thumbnail destination", ErrInvalidMetadata)
			}
		}
		...
		if track.isolated {
			if track.hostPolicy == "" {
				...
				downloadTransport = newProviderPolicyTransport(operation, track.hostPolicy, "thumbnail")
			}
		}
