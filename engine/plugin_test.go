...
func TestSignedWASMPluginPackHostAndProductIntegration(t *testing.T) {
	...
	client := newBroadTestClient(WithInstalledPlugins(installed), WithPluginPermissionApprover(approver))
	...
	focusedComposition := NewComposition[enginetest.Request](
		func(ClientProviderConfig) []providerapi.Provider[enginetest.Request] { return nil },
		func(operation providerapi.Operation, _ Request) enginetest.Request {
			return enginetest.Request{Request: operation.Request}
		},
		ProviderHooks{},
	)
	focused := NewClient(focusedComposition, WithInstalledPlugins(installed), WithPluginPermissionApprover(approver))
