...
func (operation *operation) sectionDownloadSelections(
	...
	if len(selections) == 0 {
		return "", 0, fmt.Errorf("%w: no selected formats for section", ErrUnsupported)
	}
	if err := operation.validateCredentialIsolatedDispatch(selections, true); err != nil {
		return "", 0, err
	}
...
func sectionInputsFromSelections(selections []mediaformat.Selection) ([]ffmpeg.SectionInput, error) {
	...
	for _, selection := range selections {
		...
		case "https", "http", "m3u8_native", "http_dash_segments":
			// Delegable to ffmpeg.
		default:
			return nil, fmt.Errorf("%w: This format cannot be partially downloaded. Aborting", ErrUnsupported)
		}
		if selection.CredentialIsolated {
			return nil, fmt.Errorf("%w: section download cannot enforce credential-isolated media", ErrTransportIsolation)
		}
	...
	if len(inputs) == 0 {
		return nil, fmt.Errorf("%w: no section inputs", ErrUnsupported)
	}
	return inputs, nil
}
