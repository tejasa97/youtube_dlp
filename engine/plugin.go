...
func (candidate *installedPluginExtractor) Extract(ctx context.Context, request providerapi.Request) (Extraction, error) {
	...
	if err != nil {
		return Extraction{}, mapPluginExtractorError(err)
	}
	...
	return Media(value.NewInfo(object)), nil
}

func NewInstalledPluginProvider(installed *InstalledPlugin, approver PluginPermissionApprover) providerapi.Provider[providerapi.Request] {
	return &installedPluginExtractor{installed: installed, approver: approver}
}
...
func mapPluginExtractorError(err error) error {
	...
	if errors.As(err, &remote) {
		...
		case plugin.RemoteAuthentication:
			return fmt.Errorf("%w: plugin authentication", ErrAuthentication)
		case plugin.RemoteUnavailable:
			return fmt.Errorf("%w: plugin unavailable", ErrUnavailable)
		case plugin.RemoteInvalidMetadata:
			return fmt.Errorf("%w: plugin metadata", ErrInvalidMetadata)
		}
	}
	return err
}
